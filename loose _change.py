def loose_change(cents):
    assert isinstance(cents, int) or isinstance(cents, float)

    change_dict = {'Nickels': 0, 'Pennies': 0, 'Dimes': 0, 'Quarters': 0}
    if cents > 0:
        change_dict['Quarters'] = cents // 25
        change_dict['Dimes'] = cents % 25 // 10
        change_dict['Nickels'] = cents % 25 % 10 // 5
        change_dict['Pennies'] = cents % 25 % 10 % 5 // 1
